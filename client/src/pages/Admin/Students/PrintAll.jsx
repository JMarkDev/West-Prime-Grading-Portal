...
const PrintAllGrades = ({ students, contentRef }) => {
  const userData = useSelector(getUserData);

  return (
    <div ref={contentRef} className="font-sans text-[14px]">
      {students?.map((student, studentIndex) =>
        student.academicRecords.map((record, recordIndex) => (
          <div
            key={`${studentIndex}-${recordIndex}`}
            className="w-[8.5in] h-[11in] pr-[.8in] pb-[.8in] pl-[.8in] pt-[0.5in]"
            style={{
              pageBreakAfter:
                studentIndex < students.length - 1 ||
                recordIndex < student.academicRecords.length - 1
                  ? "always"
                  : "auto",
