...
const UpdateSchoolYear = ({ id, showModal, setShowModal }) => {
  const { register, handleSubmit, reset } = useForm();
  const dispatch = useDispatch();
  const toast = useToast();
  const schoolYear = useSelector(getSchoolYearById);
  const { loading } = useSelector((state) => state.schoolYears);

  const [schoolYearError, setSchoolYearError] = useState("");

  useEffect(() => {
    dispatch(fetchSchoolYearById(id));
  }, [id, dispatch]);

  const onSubmit = async (data) => {
    dispatch(updateSchoolYear({ id, toast, data }))
      .unwrap()
      .then(() => {
        reset();
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-md bg-white rounded-xl shadow-lg p-6">
        <h1 className="text-lg font-bold text-gray-800">Update School Year</h1>
        <button
          type="button"
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-4 space-y-4">
          ...
          <div className="flex justify-end gap-4">
            ...
            <button
              ...
              disabled={loading}
            >
              {loading ? "Update..." : "Update School Year"}
            </button>
          </div>
        </form>
      </div>
